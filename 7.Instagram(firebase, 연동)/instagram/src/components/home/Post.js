...
function Post({post, updatePostData, deletePostData}) {
    ...
    useEffect(() => {
        fetch(`http://localhost:8080/comment/${post.postId}`)
    ...
    const onClickLike = () => {
        ...
        fetch(`http://localhost:8080/post/like/${post.postId}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                updatePostData(data);
            })
    ...
    const onClickDeletePost = () => {
        ...
        fetch(`http://localhost:8080/post/${post.postId}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                deletePostData(post);
            })
