...
function Post({post, updatePostData, deletePostData}) {
    ...
    useEffect(() => {
        fetch(`https://instagram2-spring.herokuapp.com/comment/${post.postId}`)
    ...
    const submitComments = (event) => {
        if(event.key === "Enter") {
            ...
            if(comment !== null && comment !== undefined) {
                ...
                let requestUrl = fixComment === 0 ? "https://instagram2-spring.herokuapp.com/comment":`https://instagram2-spring.herokuapp.com/comment/${fixComment}`;
    ...
    const onClickLike = () => {
        ...
        fetch(`https://instagram2-spring.herokuapp.com/post/like/${post.postId}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                updatePostData(data);
            })
    ...
    const onClickDeletePost = () => {
        ...
        fetch(`https://instagram2-spring.herokuapp.com/post/${post.postId}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                deletePostData(post);
            })
    ...
    const onClickDeleteComment = (commentId) => {
        ...
        fetch(`https://instagram2-spring.herokuapp.com/comment/${commentId}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                let items = [...commentList];
